...
$(document).ready(function(){
    ...
    $.ajax({
        type: "GET",
        url: 'http://localhost/cinema/user/get_client_information.php',
        data: {id: id_user},
        dataType: 'json',
        success: function(data) 
        {
            ...
            (
                ...
                <tr>\
                    <td>Date of birth: </td>\
                    <td>' + normal_date(data[0].date_of_birth) + '</td>\
                </tr>\
                <tr>\
                ...
                <tr>\
                    <td>Address: </td>\
                    <td>' + data[0].address + ', ' + data[0].city + '</td>\
                </tr>'
                // <tr>\
    ...
    $(document).on('click','.cancel',function()
    {
        $.ajax({
            type: "GET",
            url: 'http://localhost/cinema/user/delete_client_reservation.php',
    ...
    function fill_reservation_table()
    {
        $.ajax({
        type: "GET",
        url: 'http://localhost/cinema/user/get_client_reservations.php',
        data: {id: id_user},
        dataType: 'json',
        success: function(data) 
        {
            ...
            {       
                ...
                $.each(data, function(key,value)
                {
                    ...
                    (
                        '\
                            <tr>\
                                <td> <a href="movie.php?id='+ value.movie_id +'">'+ value.movie_name +'</a></td>\
                                <td>'+ normal_date_and_time(value.projection_date) +'h</td>\
                                <td>'+ value.cinema +'</td>\
                                <td>'+ value.name +'</td>\
                                <td>'+ value.row_seat +'</td>\
                                <td>'+ value.number_seat +'</td>\
                                <td> <button class="cancel" data-idseats="'+ value.id +'"> cancel </button> </td>\
                            </tr>\
                        '
                    );
