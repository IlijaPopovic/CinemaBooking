$(document).ready(function () {
  ...
  $.ajax({
    type: "get",
    url: "http://localhost/CinemaBooking/cinema/user/get_cinema.php",
    data: { id: id_cinema },
    dataType: "json",
    success: function (data) {
      ...
      if (id_admin != "no_logged_admin") {
        var cinema_change =
          '<p><button class="change_cinema" data-value="' +
          data[0].id +
          '">change</button></p><p><button class="delete_cinema" data-value="' +
          id_cinema +
          '">delete</button></p>';
      } else {
        var cinema_change = "";
  ...
  $.ajax({
    type: "get",
    url: "http://localhost/CinemaBooking/cinema/user/get_halls_for_cinema.php",
    data: { id: id_cinema },
    dataType: "json",
    success: function (data) {
      console.log(data);
      ...
      $.each(data, function (key, value) {
        if (id_admin != "no_logged_admin") {
          var delete_and_change_hall =
            '<p><button class="delete_hall" data-value="' +
            value.id +
            '">delete</button></p><p><button class="change_hall" data-value="' +
            value.id +
            '">change</button></p>';
        } else {
          var delete_and_change_hall = "";
  ...
  $(document).on("click", ".delete_hall", function () {
    ...
    $.ajax({
      type: "GET",
      url: "http://localhost/CinemaBooking/cinema/user/delete_hall.php",
  ...
  $(document).on("click", ".delete_cinema", function () {
    ...
    $.ajax({
      type: "GET",
      url: "http://localhost/CinemaBooking/cinema/user/delete_cinema.php",
