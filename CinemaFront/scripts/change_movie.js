...
$(document).ready(function(){
    ...
    $.ajax({
        type: "GET",
        url: 'http://localhost/cinema/user/get_movie.php',
        data: {id: id_movie},
        dataType: 'json',
    ...
    $('.insert').on('click',function()
    {
        if(is_all_inserted())
        {
            ...
            $.ajax({
                url:'http://localhost/cinema/user/change_movie.php',
                type:'post',
                data:post_data,
                dataType: "json",
                contentType: false,
...
{
    if( !$('#name').val() || !$('#description').val() || !$('#lenght').val() || !$('#language').val() || !$('#trailer_youtube').val() || !$('#genre').val() || !$('#actors').val() || isNaN($('#lenght').val()) || !date_check())
    {
        return false;
    }
    else
