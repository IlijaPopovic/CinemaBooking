...
$.ajax({
  type: "GET",
  url: "http://localhost/CinemaBooking/cinema/user/get_information_about_reservation.php",
      ...
      for (y = 0; y < data[0].number_seats; y++) {
        ...
        $(".seats tbody tr:last").append(
          '<td data-br-seats="' +
            ...
            "/" +
            sy +
            '"><i class="fas fa-couch"></i></td>'
        );
      }
    }
    $(".seats tbody").append(
      '<tr><td>&nbsp;</td><td data-br-seats="number" colspan="' +
        (data[0].number_seats + 1) +
        '">Screen<td></tr>'
    );
    $(".seats tbody").append(
      '<tr><td>&nbsp;</td><td data-br-seats="number" colspan="' +
      ...
      '<table>\
        ...
        "</td>\
            ...
            <tr>\
                <td>Date and time:</td>\
                <td>" +
        normal_date_and_time(data[0].projection_date) +
        'h</td>\
            </tr>\
            <tr>\
...
$(document).ready(function () {
  ...
  $.ajax({
    type: "GET",
    url: "http://localhost/CinemaBooking/cinema/user/get_reserved_places_projection.php",
    data: { id: id_projections },
    dataType: "json",
    success: function (data) {
      console.log(data);
      ...
      $.each(data, function (key, value) {
        $(
          '*[data-br-seats="' + value.row_seat + "/" + value.number_seat + '"]*'
        ).css("color", "red");
  ...
  $(document).on("click", ".reserve", function () {
    if (reserved_places !== undefined || reserved_places.leght != 0) {
      ...
      $.ajax({
        type: "GET",
        url: "http://localhost/CinemaBooking/cinema/user/insert_reservation.php",
        data: {
          client_id: id_user,
