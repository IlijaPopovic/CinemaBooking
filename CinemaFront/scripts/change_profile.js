    ...
    $('.insert').on('click',function()
        {
            if(is_all_inserted())
            {
                $.ajax({
                    type: "POST",
                    url: 'http://localhost/cinema/user/change_user.php',
                    data: 
                    {
                        'name1': $('#name').val(),
                        'surname': $('#surname').val(),
...
{
    if(!$('#name').val() || !$('#surname').val() || !$('#date_of_birth').val() || !$('#phone_number').val() || !$('#user_name').val() || !$('#mail').val() || !$('#city').val() || !$('#address').val() || !$('#password').val() || !$('#password').val()!=!$('#potvrdi_sifru').val())
    {
        return false;
    }
    else
...
function date_check()//Ne radi provera datea, proveri ovo 
{
