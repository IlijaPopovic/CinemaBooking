$(document).ready(function(){
    ...
    $('.insert').on('click',function()
    {
        if(is_all_inserted())
        {
            ...
            $.ajax({
                url:'http://localhost/cinema/user/insert_movie.php',
                type:'post',
                data:post_data,
                dataType: "json",
                contentType: false,
...
function is_all_inserted()
    {
        if(!$('#poster').val() || !$('#name').val() || !$('#description').val() || !$('#lenght').val() || !$('#language').val() || !$('#trailer_youtube').val() || !$('#genre').val() || !$('#actors').val() || isNaN($('#lenght').val()))
        {
            return false;
        }
        else
