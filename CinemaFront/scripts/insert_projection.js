$(document).ready(function () {
  ...
  $.ajax({
    type: "GET",
    url: "http://localhost/CinemaBooking/cinema/user/get_all_halls_and_movies.php",
    data: {},
    dataType: "json",
...
$(".insert").on("click", function () {
  ...
  if (is_all_inserted()) {
    $.ajax({
      type: "POST",
      url: "http://localhost/CinemaBooking/cinema/user/insert_projection.php",
      data: {
...
function checkk_date_i_tnamena() {
  ...
  if (
    polomi[0].match($regeks) &&
    polomi[1].match($regeks_za_tname) &&
    polomi.length == 2
  ) {
    return true;
  } else {
