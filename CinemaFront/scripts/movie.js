...
$(document).ready(function () {
  ...
  $.ajax({
    type: "POST",
    url:
      "http://localhost/CinemaBooking/cinema/user/get_movie.php?id=" + id_movie,
    data: {},
    dataType: "json",
    success: function (data) {
      console.log(data);
      $(".content").append("<p>" + data[0].description + "</p>");
      ...
      $(".youtube").append(data[0].trailer_youtube);

      $(".main h2").append(data[0].name);
  ...
  $.ajax({
    type: "POST",
    url:
      "http://localhost/CinemaBooking/cinema/user/get_repertoire_for_movie.php?id=" +
      id_movie,
    data: {},
    dataType: "json",
    success: function (data) {
  ...
  function print_comments() {
    $.ajax({
      type: "POST",
      url:
        "http://localhost/CinemaBooking/cinema/user/get_comments_for_movie.php?id=" +
        id_movie,
      data: {},
      dataType: "json",
      success: function (data) {
        ...
        if (data.length != 0) {
          ...
          $.each(data, function (key, value) {
            if (id_admin != "no_logged_admin") {
              var comments_del =
                '<td><button class="delete_comment" data-value="' +
                value.id +
                '">delete</button></td>';
            } else {
              var comments_del = "";
        ...
        if (value.name == selected) {
          if (id_admin != "no_logged_admin") {
            var delete_projections =
              '<td><button class="delete_projection" data-value="' +
              value.projection_id +
              '">delete</button></td>';
          } else {
            var delete_projections = "";
  ...
  $(".comment").on("click", function () {
    if ($(".new_comment").val() != "" && $(".grade").val() != "choose_grade") {
      $.ajax({
        type: "GET",
        url: "http://localhost/CinemaBooking/cinema/user/comment.php",
        data: {
          id: id_movie,
        ...
        success: function (data) {
          ...
          if (data.status == "inserted") {
            ...
            var pitanje = confirm(
              "You have already commented on this movie, do you want to delete the previous comment and post a new one?"
            );

            if (pitanje) {
              $.ajax({
                type: "GET",
                url: "http://localhost/CinemaBooking/cinema/user/delete_users_comments.php",
                data: {
                  id: id_movie,
                  client: id_user,
                },
  ...
  $(document).on("click", ".delete_projection", function () {
    ...
    $.ajax({
      type: "GET",
      url: "http://localhost/CinemaBooking/cinema/user/delete_projection.php",
  ...
  $(document).on("click", ".delete_comment", function () {
    ...
    $.ajax({
      type: "GET",
      url: "http://localhost/CinemaBooking/cinema/user/delete_comment.php",
  ...
  $(document).on("click", ".delete_movie", function () {
    $.ajax({
      type: "GET",
      url: "http://localhost/CinemaBooking/cinema/user/delete_movie.php",
